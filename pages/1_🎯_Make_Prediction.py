...
def load_model(model_type):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Read and modify the config.json
            config_path = Path(tmpdir) / 'config.json'
            with open(config_path, 'r', encoding='utf-8') as f:
                config_text = f.read()
            
            # Remove quantization_config from the config
            import re
...
# Main content
st.markdown('<h1 style="text-align: center; color: #ffffff;">🎯 Make Crop Yield Prediction</h1>', unsafe_allow_html=True)
